...
func TestReadConfig(t *testing.T) {
	...
	assert.Equal(t, 1, len(commands[0].Args))
	assert.Equal(t, "Execbeat", commands[0].Args[0])
	...
	assert.Equal(t, 1, len(commands[1].Args))
	assert.Equal(t, "Hello World", commands[1].Args[0])
